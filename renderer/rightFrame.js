...
async function loadConnection() {
  ...
  if(connectionSelector) {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8080/app/user/loadConnection", {
                      method: "GET"
      });
...
async function connectionSelectListener(selectElement) {
    ...
    if(connMap.has(selectedValue)) {
      ...
      const setSessionResponse = await fetch(`http://localhost:8080/app/user/currentSession?connName=${connMap.get(selectedValue)}`, {
                      method: "POST"
      });
      console.log("Session for " + await setSessionResponse.text());
    } else{
      console.log("No Connection!");
      const disConnResponse = await fetch("http://localhost:8080/app/user/disConnectConn", {
                      method: "GET"
      });
